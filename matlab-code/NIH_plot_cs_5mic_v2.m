...
wd = .49;
wd2 = wd/4;
lft1 = .0; % start of first col;

lft2 = lft1+wd+xpad;


ht_im = .99; % image heights
bt_im = 0.;
...
cs_im = cs_img_data_s{2}.cs_im; 
% cs_im = cs_im - mean(cs_im(:));
% cs_im = cs_im.*cs_img_data_s{2}.pixelifsampled;
cs_im = min(cs_im, 0);
% cs_im = max(cs_im, -thresh);
% imshow(cs_im-mean(cs_im(:)), [-thresh, thresh], 'Parent', ax1)

imshow(cs_im-mean(cs_im(:)), [min(cs_im(:)), max(cs_im(:))], 'Parent', ax1)
...
imshow(I_plot, [-thresh, thresh],...
    'XData', xdata, 'YData', ydata, 'Parent', ax2)
...
%%
fig_path = fullfile('/home/arnold/gradschool/proposals/nih-fall-2017/overleaf-git/figures/mu_path02.eps');
