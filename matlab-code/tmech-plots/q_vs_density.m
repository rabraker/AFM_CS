...
opts = NESTA_opts('U', U_fun, 'Ut', Ut_fun, 'verbose', 0, 'TolVar', 1e-5,...
    'alpha_v', alpv, 'alpha_h', alph, 'mu', mu, 'sigma', sigma);
...
figure(11); clf
subplot(1,4,1),
imagesc(im_og, [1, 255]), colormap('gray')

subplot(1,4,2),
imagesc(im_128, [1, 255]), colormap('gray')
title(sprintf('%d lines (ssim: %.2f, psnr: %.2f)', size(im_128_sub, 1), ssm_128, psn_128))
subplot(1,4,3)
imagesc(im_64, [1, 255]), colormap('gray')
title(sprintf('%d lines (ssim: %.2f, psnr: %.2f)', size(im_64_sub, 1), ssm_64, psn_64))
...
%         figure(11);
        ...
        colormap('gray')
        title(sprintf('CS: q=%d, frac=%.1f, (ssim: %.2f, psnr: %.2f', mu_len,...
