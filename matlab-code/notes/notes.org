...
#+LATEX: \begin{figure*}[htbp]\begin{minipage}{0.5\textwidth}
#+HTML: <figure>
...
#+LATEX: \end{minipage}\begin{minipage}{0.5\textwidth}\hfill
...
#+name: fig:dfl_time_psd
#+caption: (right) Time series. (left) PSDs showing the difference the three modifications make. The blue curves are the unmodified instrument, the orange curves are the instrument with vibration isolation and the yellow curves are the with both vibration isolation and using an external power source to run the nPoint cooling fan.
#+ATTR_LATEX: :environment subfigure :width 0.4\textwidth :align c
| file:figures/dfl_time.svg                              | file:figures/dfl_psd.svg                      |
| <<fig:dfl_time>> Time Series of the deflection signal. | <<fig:testff>> PSDs of the deflection signal. |



Figures [[fig:dfl_time_psd]] illustrate the difference. The blue time series in Fig. [[fig:dfl_time_psd]] shows what the deflection signal in our AFM looks like with the controll loops turned off, the XY-stage turned on, and the cantilevar merely resting on the surface. It is really noisy and has large periodic content. The blue curve in [[]] shows the PSD estimate (taken from 50 realizations) of the deflection signal in the same scenario. 
...
It turns out that the spike at 380 Hz in the PSD is caused by the cooling fan inside the xy stage power amplifier. If we unplug that fan, we get the yellow timeseries in Fig. [[fig:dfl_time_psd]]  which is a vast improvement over the original instrument. 
