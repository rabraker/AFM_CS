...
function [xp, up, niter] = l1qc_newton(x0, u0, A, At, b, epsilon, ...
...
%   tstart_loop = tic;
  ...
  %RABRAKER Note: This code computes the largest step size s such that 
  % ||A(x + s*dx) - b||^2 < epsilon^2. 
  %  The aeq, bqe, cqe, are abc in a quadratic equation. Sometimes,
  % the computed s is not small enough, and fep >0, by like 1e-18. This makes
  % log(-fep) complex. I've added a small fudge factor to cqe to try to
  % prevent. The s=0.99smax is theirs and evidently not enough.
  aqe = Adx'*Adx;   
  bqe = 2*r'*Adx;   
  cqe = r'*r - epsilon^2 + eps*0.01;
  smax = min(1,min([...
  ...
  if imag(smax) ~= 0
    smax = real(smax);
    fprintf('************ Smax Imaginiary. Using real part\n');
    %keyboard
  end
...
%   tend_loop = toc(tstart_loop);
%   keyboard
%   fprintf('total iteration time: %f\n',  tend_loop);
%   fprintf('CG-solve time: %f, cg_iters: %d\n', tend_cg, cgiter);
%   fprintf('CG avg time per iter: %f\n', tend_cg/cgiter);
%   fprintf('Line Search time: %f\n', tend_ls);
%   fprintf('Loops: %d\n', niter);
end
